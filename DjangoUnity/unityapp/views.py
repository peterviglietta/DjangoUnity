...
#THEY ARE ALL COMMENTED OUT SO THAT YOU CAN RUN THE SERVER TO TEST YOUR APP. I THINK THIS
#WILL BE IN A SEPARATE SCHEDULED SERVICE IN PRODUCTION, NOT IN A DJANGO VIEW???
...
dipshit = json.loads(unity_output)
print dipshit

tits = dipshit[0]

faggot = tits['getordersinfo']


def dummyView(xxx):
    return HttpResponse('titties')
    #return 'HttpResponse(unity_output)'
...
#THE saveOrders function below is just the API call that posts orders to the Quirklist database
'''
def saveOrders():
    for i in faggot:
